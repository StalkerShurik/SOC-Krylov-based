#NOTE CODE HAVE PROBLEMS AND LIKELY INCORRECT
...
def basis_arnoldi_conv(L, X, m, kernel_size):
    ...
    for j in range(m):
        w  = F.conv2d(v[j], L, padding=(kernel_size//2, kernel_size//2)).unsqueeze(dim=0)
        for i in range(j+1):
            h[:,i,j] = torch.sum(w[-1] * v[i], axis=(1,2,3))
            #w = w - torch.einsum("b,bijk->bijk", h[:,i,j].clone().detach(), v[i]).half()
            #w = w - h[:,i,j].view(h.shape[0], 1, 1, 1).clone().detach() * v[i]
            
            torch.cat((w, (w[-1] - h[:,i,j].view(h.shape[0], 1, 1, 1) * v[i]).unsqueeze(dim=0)), dim=0)

        h[:,j+1,j] = torch.linalg.norm(torch.linalg.norm(w[-1], dim=-1), dim=(-1,-2))
        ...
        new_v = w[-1] * torch.reciprocal(h[:,j+1,j]).view(h.shape[0], 1, 1, 1)
